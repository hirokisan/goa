...
import (
	...
	genapp "github.com/hirokisan/goa/goagen/gen_app"
...
var _ = Describe("BuildEncoders", func() {
	...
	Context("with a single definition using a single known MIME type for encoding", func() {
		...
		It("generates a map with a single entry", func() {
			...
			Ω(jd.PackagePath).Should(Equal("github.com/hirokisan/goa"))
	...
	Context("with a definition using a custom decoding package", func() {
		const packagePath = "github.com/hirokisan/goa/design" // Just to pick something always available
	...
	Context("with a definition using a custom decoding package from hirokisan", func() {
		const packagePath = "github.com/hirokisan/goa/encoding/gogoprotobuf"
